...
RUN apt-get -y update && \
    apt-get -y install curl build-essential gdb gcc-multilib m4 m4-doc
#libpixman-1-dev zlib1g-dev libglib2.0-dev libtool-bin libsdl1.2-dev 
#make gmp
...
RUN cd ./gmp-${GMP_VERSION} \
    ...
    && make -s -j8  && make check -s -j8 && make install && rm -rf ../gmp-${GMP_VERSION} && ldconfig
...
RUN cd ./mpfr-${MPFR_VIRSION} \
    && ./configure --prefix=/usr/local \
    && make -s -j8 && make check -s -j8 && make install && rm -rf ../mpfr-${MPFR_VIRSION} && ldconfig
#make mpc
ARG MPC_VIRSION="0.9"   
...
RUN cd ./mpc-${MPC_VIRSION} \
    ...
    && make -s -j8 && make check -s -j8 && make install && rm -rf ../mpc-${MPC_VIRSION} && ldconfig
#make binutils
...
##make gdb
#ARG GDB_VERSION="gdb-7.3.1"
#RUN curl -SL http://ftpmirror.gnu.org/gdb/${GDB_VERSION}.tar.bz2 | tar xj -C ./  
#RUN cd ./${GDB_VERSION} \
#    && ./configure --prefix=/usr/local --target=i386-jos-elf --program-prefix=i386-jos-elf- --disable-werror\
#    && make -s -j8 all && make install && rm -rf ../${GDB_VERSION}
